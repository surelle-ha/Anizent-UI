<template>
    <Panel :pt="{
        ...
        <div class="card px-4">
            <h2 class="text-right mr-6">Query: {{ route.query.query }}</h2>
            <Carousel :value="series_layer_1" :numVisible="6" :numScroll="3">
                <template #item="slotProps" >
                    <div class="episode-card border-1 surface-border border-round m-2 p-3 cursor-pointer" @click="openDetails(slotProps.data.id)" style="width:300px;">
                        <div class="mb-3">
...
watch(url_query, (newQuery) => {
    if (newQuery && newQuery !== '') {
        getQueriedSeries(newQuery);
    }
});

const getQueriedSeries = async (query) => {
    isLoading.value = true;
    try {
        const response = await axios.get(`${process.env.VUE_APP_MEDIA_API_URL}/anime/gogoanime/${query}`);
        series_layer_1.value = response.data.results;
        console.log(series_layer_1.value)
    } catch (error) {
...
onMounted(() => {
    if (url_query.value) {
        getQueriedSeries(url_query.value);
    }
});
